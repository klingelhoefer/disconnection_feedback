...
explainer_wb_ge = "<strong> Allgemeines Wohlbefinden</strong>: Diese Variable erfasst, wie Sie sich im Moment der Befragung im Allgemeinen gefühlt haben und wurde mit einer Frage gemessen, die danach fragte, wie gut bzw. schlecht Sie sich gefühlt haben. Die Werte können von 1 bis 7 reichen. Hohe Werte bedeuten ein hohes Wohlbefinden und niedrige Werte niedriges Wohlbefinden. Die Darstellung im Diagramm zeigt den Tagesdurchschnitt."

explainer_wb_energy = "<strong>Energie</strong>: Diese Variable erfasst, wie energiegeladen oder aktiviert Sie sich gefühlt haben. Hohe Werte auf der Skala von 1 bis 7 zeigen, dass Sie sich voller Energie gefühlt haben. Die Darstellung im Diagramm zeigt den Tagesdurchschnitt."

explainer_wb_stress = "<strong>Stress</strong>: Diese Frage misst, wie gestresst Sie sich im Moment der Befragung gefühlt haben. Hohe Werte auf der Stress-Skala von 1 bis 7 zeigen, dass Sie sich stärker gestrest gefühlt haben. Die Darstellung im Diagramm zeigt den Tagesdurchschnitt."

explainer_wb_connect = "<strong>Verbundenheit</strong>: Die Frage nach Verbundenheit mit anderen soll erfassen, wie eingebunden Sie sich in diesem Moment in Ihr soziales Umfeld gefühlt haben. Hohe Werte auf der Skala von 1 bis 7 zeigen, dass Sie sich stärker mit anderen Verbunden und nicht einsam gefühlt haben. Die Darstellung im Diagramm zeigt den Tagesdurchschnitt."

explainer_screentime_work = "<strong>Arbeits-/Lernbezogene Mediennutzung</strong>: Diese Frage erfasst, wie viele Minuten der letzten zwei Stunden Sie mit digitalen Medien für Arbeit oder Schule/Studium verbracht haben. Die Darstellung im Diagramm zeigt an wie viele Minuten Mediennutzung im Durchschnitt für die letzten zwei Stunden angegeben wurde."

explainer_screentime_leisure = "<strong>Freizeitbezogene Mediennutzung</strong>: Diese Frage erfasst, wie viele Minuten der letzten zwei Stunden Sie digitale Medien zur Unterhaltung, Entspannung, privaten sozialen Interaktionen, etc. genutzt haben. Die Darstellung im Diagramm zeigt an wie viele Minuten Mediennutzung im Durchschnitt für die letzten zwei Stunden angegeben wurde."

explainer_procra = "<strong>Prokrastination</strong>: Prokrastination misst das Ausmaß, zu dem Sie Dinge, die Sie erledigen wollten, ohne guten Grund herausgezögert haben. Prokrastination kann negative Auswirkungen haben, z. B. durch Schuldgefühle aufgrund des Aufschiebens. Hohe Werte bedeuten, dass Aufgaben verstärkt verzögert wurden. Die Darstellung im Diagramm zeigt den Tagesdurchschnitt."

explainer_disco = "<strong>Einschränkung digitaler Medien</strong>: Diese Variable zeigt den Durchschnitt zwischen allen Kategorien digitaler Medien, auf die Sie verzichtet haben, z. B. Geräte oder bestimmte Interaktionen. Werte näher an 1 zeigen an, dass auf viele Kategorien verzichtet wurde und Werte näher an 0 zeigen, dass auf wenige Kategorien verzichtet wurde. Die Darstellung im Diagramm zeigt den Tagesdurchschnitt."

explainer_goal_conf = "<strong>Zielkonflikt</strong>: Diese Frage zielte darauf ab zu erfassen, ob Sie empfinden, dass Ihre Mediennutzung im Konflikt mit anderen wichtigen Zielen stand. Manchmal halten uns digitale Medien davon ab, Dinge zu erledigen, die wir eigentlich machen sollten oder wollen, z. B. E-Mails können vom Entspannen abhahlten oder Kurzvideos vom Lernen. Hohe Werte auf der Skala von 1 bis 7 zeigen ein hohes Ausmaß von Zielkonflikten an. Die Darstellung im Diagramm zeigt den Tagesdurchschnitt."
...
server = function(input, output, session) {
  ...
  #rendering breaks
  output$login_instructions = renderUI({if(clicked_r() == 0)HTML("<h4>Bitte geben Sie ihre Login-Daten ein</h4><p>Geben Sie Ihre Pin ohne führende Nullen an</p>")})
      ...
      tagList(
        # Input: slecting smoothing
         selectInput(inputId = "mean_selection_1",
                label = paste0('Optional: Vergleich mit Gesamtmittelwert für ', 
